...
public class DonationUpdateUI extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					DonationUpdateUI frame = new DonationUpdateUI();
	 ...
	 * Create the frame.
	 */
	public DonationUpdateUI() {
		setTitle("Updating Donation Operation Informations\r\n");
		setIconImage(Toolkit.getDefaultToolkit().getImage(DonationUpdateUI.class.getResource("/images/logo1.jpg")));
		...
		JLabel label_1 = new JLabel("dSSN");
		...
		JLabel label_2 = new JLabel("Date Of Donation");
		...
		btn.setBounds(171, 399, 260, 40);
		contentPane.add(btn);

		JPanel panel = new JPanel();
		panel.setLayout(null);
		panel.setBackground(new Color(165, 42, 42));
		panel.setBounds(460, 0, 296, 130);
		contentPane.add(panel);

		JLabel lblUpdatingDonationOperation = new JLabel("Updating Donation Operation Informations\r\n");
		lblUpdatingDonationOperation.setHorizontalAlignment(SwingConstants.CENTER);
		lblUpdatingDonationOperation.setForeground(Color.WHITE);
		lblUpdatingDonationOperation.setFont(new Font("Century", Font.PLAIN, 14));
		lblUpdatingDonationOperation.setBounds(0, 11, 296, 108);
		panel.add(lblUpdatingDonationOperation);

		JTextArea txtrImportantNoteyou = new JTextArea();
		txtrImportantNoteyou.setText(
				"IMPORTANT NOTE:\r\n-you must fill the id for the donation operation \r\n-only fill the values that you want to change\r\n-and leave the others empty");
		...
		txtrImportantNoteyou.setBounds(481, 166, 265, 177);
		...
		separator.setBounds(460, 11, 11, 449);
		contentPane.add(separator);
		
		 lbl = new JLabel("ID must be filled");
		lbl.setFont(new Font("Tahoma", Font.PLAIN, 10));
		lbl.setBounds(170, 382, 261, 14);
	...
	private void jButtonActionPerformed(java.awt.event.ActionEvent e) throws SQLException, ParseException {
		...
		else if( (tf5.getText().compareTo("") != 0 && tf5.getText().matches("[a-zA-Z]+")==false )|| (tf6.getText().compareTo("") != 0 && tf6.getText().matches("[a-zA-Z]+")==false )) {
			lbl.setText("hospital and doctor names must be strings");
			lbl.setVisible(true);
		}
		else if(dr.search(Integer.valueOf(tf1.getText()), 3) == 0){
			lbl.setText("this id doesn't exist");
			lbl.setVisible(true);
		}
		else if(tf2.getText().compareTo("") != 0 && dr.search(Integer.valueOf(tf2.getText()), 1) == 0){
		...
		if (tf2.getText().compareTo("") != 0) {
			updatedColumns.add("dSSN");
			updatedValues.add(tf2.getText());
		}
		if (dateChooser.getDate() != null ){
			getDateFromUI();
			updatedColumns.add("DateofDonation");
		...
		if((!updatedColumns.isEmpty()) && !updatedValues.isEmpty()) 
			dr.update(3, id, updatedColumns, updatedValues);
